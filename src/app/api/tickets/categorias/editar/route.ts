  ...
  return NextResponse.json(
    {
      ...
      message: categoryData?.detail || categoryData?.message || 'Error al editar categoría'
    },
    { status }
  )
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    // Update category
    const categoryRes = await updateCategory(baseUrl, token, categoryData)
    ...
    // Success response
    return NextResponse.json(
      { message: 'Categoría editada con éxito', data: categoryResData },
      { status: 200 }
    )
  } catch (err) {
    console.error('Error en /api/tickets/categorias/editar:', err)
