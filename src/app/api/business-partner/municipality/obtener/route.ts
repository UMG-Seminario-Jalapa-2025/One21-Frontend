...
export async function GET() {
  try {
    ...
    const res = await fetch(`${baseUrl}partners/municipalities`, {
      method: 'GET',
    ...
    if (!res.ok) {
      return NextResponse.json(
        { step: 'municipality_get', message: 'Error al obtener municipios' },
        { status: res.status }
      )
    }
    ...
    console.error('❌ Error en /api/municipality/obtener:', err)
