...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!tokenCookie?.value) {
      return new Response(JSON.stringify({ message: 'Token no encontrado en cookies' }), { status: 401 })
    }

    const token = tokenCookie.value

    const res = await fetch(`${baseUrlTemp}partners/partners/${params.id}`, {
      method: 'GET',
      headers: {
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({ message: msg || 'Error al obtener socio' }), { status: res.status })
    }
